/**
A small software rasterizer for 2D vector graphics. Works with complex polygons, trapezoids, and thin lines.

Taken from https://github.com/dayllenger/beamui/blob/master/src/beamui/graphics/swrast.d
commit: 95194f767ebaf93272de1133bf00da82a22f88bf
...
bool fzero2(float a) { return -0.00999999f < a && a < 0.00999999f; }
bool fequal2(float a, float b) { return fabs(a - b) < 0.0099999f; }
...
RectF computeBoundingBox(const Vec2[] points)
{
    RectF r = RectF(float.max, float.max, -float.max, -float.max);
    foreach (p; points)
    {
        if (r.left > p.x)
            r.left = p.x;
...
const(T[]) slice(T)(ref const Array!T array)
{
    return array.length > 0 ? (&array.front())[0 .. array.length] : null;
}
    ...
    in(plotter)
{
    ...
    foreach (i, ref ctr; ctrs[0 .. contours.length])
    {
        ...
        if (ceCount > 0)
        {
            const before = cast(uint)pts.length;
            clip_poly(points[ctr.start .. ctr.start + ctr.len], clipEdges[0 .. ceCount], pts);
            ctr.start = before;
            ctr.len = cast(uint)pts.length - before;
            ctr.updated = true;
        }
    }
...
void clip_poly(const(Vec2)[] input, const Vec2[2][] clipEdges, ref Array!Vec2 output)
{
    ...
    static Array!Vec2 tmp;

    const olen = output.length;
    Array!Vec2* writer = (clipEdges.length % 2 == 1) ? &output : &tmp;
    ...
    foreach (ref edge; clipEdges)
    {
        ...
        foreach (i; 0 .. input.length)
        {
            ...
            {
                writer.insertBack(intersection(edge, s, e));
                writer.insertBack(e);
            }
...
public struct HorizEdge
{
    ...
    static bool isValidTrapezoid(HorizEdge top, HorizEdge bot) nothrow @nogc
    {
